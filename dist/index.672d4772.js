...
const displayMovements = function(movements, sorted = false) {
    ...
    mov.forEach((el, i)=>{
        ...
        const html = `<div class="movements__row">
          ...
          <div class="movements__value">${el}\u{20AC}</div>
        </div>`;
        containerMovements.insertAdjacentHTML("afterbegin", html);
    });
};
const blanceSummary = function(user) {
    const balanceIn = user.movements.filter((mov)=>mov > 0).reduce((acc, mov)=>acc + mov);
    labelSumIn.innerHTML = `${balanceIn}\u{20AC}`;
    const balanceOut = user.movements.filter((mov)=>mov < 0).reduce((acc, mov)=>acc + mov);
    labelSumOut.innerHTML = `${Math.abs(balanceOut)}\u{20AC}`;
    const interest = user.movements.filter((mov)=>mov > 0).map((interest)=>interest * user.interestRate / 100).reduce((acc, int)=>acc + int);
    labelSumInterest.innerHTML = `${interest}\u{20AC}`;
};
const balancePrint = function(user) {
    user.balance = user.movements.reduce((acc, mov)=>acc + mov);
    labelBalance.innerHTML = `${user.balance}\u{20AC}`;
};
const userName = function(acc) {
    acc.forEach(function(acc) {
        acc.username = acc.owner.toLocaleLowerCase().split(" ").map((user)=>user[0]).join("");
    });
};
userName(accounts);
let user;
...
btnTransfer.addEventListener("click", (e)=>{
    ...
    if (amount > 0 && user.balance > amount && reciever && reciever.username !== user.username) {
...
btnLoan.addEventListener("click", (e)=>{
    ...
    if (amount > 0 && user.movements.some((e)=>e >= amount * 0.1)) setTimeout(()=>{
        user.movements.push(amount);
        balancePrint(user);
        displayMovements(user.movements);
        blanceSummary(user);
